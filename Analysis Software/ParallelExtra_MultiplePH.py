"""
Sequential and Parallel Timing Analysis with Excel Summary Output
Date: 19 March 2026
...
Brief Description
-----------------
Benchmarks sequential and parallel image processing for two datasets, exports
timing results to Excel, and generates summary plots for reporting.
"""
...
# If both datasets are stored in the same folder, set both paths equal.
datasets = {
    "pH 12": {
        "folder": folder_ph12,
        "start_index": 1,
...
n_runs = 10
max_workers = None  # Lets ThreadPoolExecutor choose automatically
...
excel_path = os.path.join(
    os.path.dirname(folder_ph12),
    "parallel_processing_timing_summary.xlsx"
)
...
def find_image_path(folder, i):
    """Return the first matching image path using 5-digit or 4-digit naming."""
    candidates = [
...
def get_output_folders(base_folder, dataset_label):
    """Create and return sequential and parallel output folders."""
...
def process_image(folder, i, output_folder):
    """Read an image, isolate blue regions, reduce noise, and save output."""
...
def run_sequential(folder, start_index, end_index, output_folder):
    """Process images sequentially and return elapsed time."""
...
def create_excel_workbook(all_results, save_path):
    """Save raw timings and summary statistics into a formatted Excel workbook."""
    wb = Workbook()
    wb.remove(wb.active)
...
def make_summary_table_plot(all_results):
    """Create a summary table plot for report use."""
    rows = []
    ...
    plt.title(
        "Runtime Summary for Sequential and Parallel Image Processing",
        fontsize=14,
        pad=16
    )
    plt.tight_layout()
...
def make_grouped_bar_plot(all_results):
    """Create grouped bar charts with error bars and summary annotations."""
    ...
    ax.set_title(
        "Comparison of Mean Runtime for Sequential and Parallel Processing",
        fontsize=14
    )
    ...
    for dataset_label in dataset_names:
        ...
        summary_lines.append(
            f"{dataset_label}: {speedup:.2f}×, {improvement:.1f}% faster"
        )
...
def make_per_run_plot(all_results):
    """Plot repeated sequential and parallel runtimes for each dataset."""
