"""
Background Subtraction and Frame Brightness Analysis
Date: 19 March 2026
...
Brief Description
-----------------
Loads a sequence of grayscale frames, computes a mean background image,
performs background subtraction, saves processed outputs, and plots framewise
brightness and activity metrics.
"""
...
# Signed difference preserves whether a pixel is brighter or darker than average
diff_images = images - mean_image

# Absolute difference is used for activity detection
...
for i, diff_img in enumerate(diff_images):
    # Shift values into visible range for saving
    vis = diff_img + 128
...
# Save absolute-difference images
...
active_intensity_sum = np.sum(
    np.where(abs_diff_images > threshold, abs_diff_images, 0),
    axis=(1, 2)
)
